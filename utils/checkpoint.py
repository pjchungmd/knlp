"""
Taken from: https://github.com/pudae/kaggle-understanding-clouds/blob/master/kvt/utils/checkpoint.py
"""
...
def get_last_checkpoint(checkpoint_dir):
    checkpoints = [checkpoint
                   ...
                   if checkpoint.startswith('epoch_') and checkpoint.endswith('.bin')]
    if checkpoints:
...
def save_checkpoint(config, model, optimizer, epoch, step=0, keep=None, weights_dict=None, name=None):
    ...
    if name:
        checkpoint_path = os.path.join(checkpoint_dir, '{}.bin'.format(name))
    else:
        checkpoint_path = os.path.join(checkpoint_dir, 'epoch_{:04d}.bin'.format(epoch))
